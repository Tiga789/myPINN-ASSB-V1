...
def rescale_param_list(nn, param_list):
    return [nn.rescale_param(param_list[i], i) for i in range(len(nn.resc_params))]
...
def rescale_var_list(nn, var_list, dummyVal=np.float64(0.0)):
    ...
    if len(var_list) == 2:
        ...
        var_list_resc_full[nn.ind_r] = (
            dummyVal * np.ones(var_list_resc_full[nn.ind_t].shape, dtype=np.float64) / nn.params["rescale_R"]
        )
...
def make_data_dict(dataFolder, param_list=None):
    ...
    data_phie = np.load(os.path.join(dataFolder, f"data_phie{param_string}.npz"))
    ...
    data_phis_c = np.load(os.path.join(dataFolder, f"data_phis_c{param_string}.npz"))
    ...
    data_cs_a = np.load(os.path.join(dataFolder, f"data_cs_a{param_string}.npz"))
    ...
    data_cs_c = np.load(os.path.join(dataFolder, f"data_cs_c{param_string}.npz"))
...
def make_var_params_from_data(nn, data_dict):
    ...
    params_dict["phie_unr"] = [data_dict["params_phie"][:, i] for i in range(nparams)]
    params_dict["phie"] = rescale_param_list(nn, params_dict["phie_unr"])
    var_dict["phie_unr"], var_dict["phie_unr_full"] = var_from_x(data_dict["var_phie"])
    var_dict["phie"], var_dict["phie_full"] = rescale_var_list(nn, var_dict["phie_unr"])

    params_dict["phis_c_unr"] = [data_dict["params_phis_c"][:, i] for i in range(nparams)]
    params_dict["phis_c"] = rescale_param_list(nn, params_dict["phis_c_unr"])
    var_dict["phis_c_unr"], var_dict["phis_c_unr_full"] = var_from_x(data_dict["var_phis_c"], nn.params["Rs_c"])
    var_dict["phis_c"], var_dict["phis_c_full"] = rescale_var_list(nn, var_dict["phis_c_unr"])

    params_dict["cs_a_unr"] = [data_dict["params_cs_a"][:, i] for i in range(nparams)]
    params_dict["cs_a"] = rescale_param_list(nn, params_dict["cs_a_unr"])
    var_dict["cs_a_unr"], var_dict["cs_a_unr_full"] = var_from_x(data_dict["var_cs_a"], nn.params["Rs_a"])
    var_dict["cs_a"], var_dict["cs_a_full"] = rescale_var_list(nn, var_dict["cs_a_unr"])

    params_dict["cs_c_unr"] = [data_dict["params_cs_c"][:, i] for i in range(nparams)]
    params_dict["cs_c"] = rescale_param_list(nn, params_dict["cs_c_unr"])
    var_dict["cs_c_unr"], var_dict["cs_c_unr_full"] = var_from_x(data_dict["var_cs_c"], nn.params["Rs_c"])
    var_dict["cs_c"], var_dict["cs_c_full"] = rescale_var_list(nn, var_dict["cs_c_unr"])
...
def pinn_pred(nn, var_dict, params_dict):
    ...
    with torch.no_grad():
        ...
        phie_rescaled = nn.rescalePhie(phie_nonrescaled, *var_dict["phie_unr"], *params_dict["phie_unr"])
        pred_dict["phie"] = _to_numpy(phie_rescaled)

        out = nn.model(var_dict["phis_c_full"] + params_dict["phis_c"])
        phis_c_nonrescaled = out[nn.ind_phis_c]
        phis_c_rescaled = nn.rescalePhis_c(phis_c_nonrescaled, *var_dict["phis_c_unr"], *params_dict["phis_c_unr"])
        pred_dict["phis_c"] = _to_numpy(phis_c_rescaled)

        out = nn.model(var_dict["cs_a_full"] + params_dict["cs_a"])
        cs_a_nonrescaled = out[nn.ind_cs_a]
        cs_a_rescaled = nn.rescaleCs_a(cs_a_nonrescaled, *var_dict["cs_a_unr_full"], *params_dict["cs_a_unr"])
        pred_dict["cs_a"] = _to_numpy(cs_a_rescaled)

        out = nn.model(var_dict["cs_c_full"] + params_dict["cs_c"])
        cs_c_nonrescaled = out[nn.ind_cs_c]
        cs_c_rescaled = nn.rescaleCs_c(cs_c_nonrescaled, *var_dict["cs_c_unr_full"], *params_dict["cs_c_unr"])
        pred_dict["cs_c"] = _to_numpy(cs_c_rescaled)
...
def pinn_pred_struct(nn, params_list):
    ...
    t_a = np.reshape(
        np.repeat(np.repeat(np.repeat(t_test_a, n_r, axis=1), n_par, axis=2), n_par, axis=3),
        (n_r * n_t * n_par * n_par, 1),
    ).astype("float64")
    r_a = np.reshape(
        np.repeat(np.repeat(np.repeat(r_test_a, n_t, axis=0), n_par, axis=2), n_par, axis=3),
        (n_r * n_t * n_par * n_par, 1),
    ).astype("float64")
    ...
    t_c = np.reshape(
        np.repeat(np.repeat(np.repeat(t_test_c, n_r, axis=1), n_par, axis=2), n_par, axis=3),
        (n_r * n_t * n_par * n_par, 1),
    ).astype("float64")
    r_c = np.reshape(
        np.repeat(np.repeat(np.repeat(r_test_c, n_t, axis=0), n_par, axis=2), n_par, axis=3),
        (n_r * n_t * n_par * n_par, 1),
    ).astype("float64")

    params_r_unr = [
        np.ones((n_t * n_r, 1), dtype=np.float64) * entry for entry in params_list
    ]
